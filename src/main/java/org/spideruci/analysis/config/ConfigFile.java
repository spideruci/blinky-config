...
import org.spideruci.analysis.config.utils.Logger;
...
public class ConfigFile {
  ...
  public static ConfigFile create(String fileName) {
    return create(fileName, new Logger());
  }
  
  public static ConfigFile create(String fileName, final Logger log) {
    File configfile = getFile(fileName, log);
    ConfigFile configFile = new ConfigFile(configfile);
    return configFile;
  }
  
  private static File getFile(String fileName, Logger log) {
    ...
    if(configNotExists || configNotFile || configIsDirectory) {
      log.debug("Config doesn't exist? %B", configNotExists);
      log.debug("Config not a file? %B", configNotFile);
      log.debug("Config is a directory? %B", configIsDirectory);
      
      final String invalidFileMessage = String.format("No valid config file at %s", file.toString());
      log.error(invalidFileMessage);
      throw new RuntimeException(invalidFileMessage);
    }
    
    log.info("Valid config file available at %s", file.toString());
    return file;
  }
  ...
  public static void logConfig(Map<String, ?> map, Logger log) {
